...
## Open / Unresolved
|- **CR-6 (RESOLVED 2026-09-10): Render poll false convergence on SPA skeleton cells** — `worker.py:420-541`. The render poll's convergence gate had three signals (url stable, text >100 chars, posts > 0), but Tumblr's SPA renders `div[data-cell-id*="-post-"]` skeleton elements immediately on page load with the same attribute as real posts. The gate converged on skeleton cell count alone (posts=17, text_len=500 = nav menu only), but the HTML snapshot captured only empty skeletons, yielding 0 extracted usernames. **Fix applied (commit 4f88f50):** added `author_links` count via `a[rel="author"]` selector to the render poll JS expression; convergence gate now requires `author_links > 0` (skeleton cells have zero author links). Added debug logging of `author_links` in converged and incomplete log paths.
