...
## Open / Unresolved
|- **CR-6 (NEW): Render poll false convergence on SPA skeleton cells** — `worker.py:420-541`. The render poll's convergence gate keys off `div[data-cell-id*="-post-"]` element count, but Tumblr's SPA renders skeleton placeholder elements with that attribute immediately on page load — before actual post content (author links, post bodies) loads asynchronously. Every render poll log shows `text_len=500` (just nav menu "Home 0 Explore Communities...") and `posts=17-18` (skeleton cells), triggering `render CONVERGED` — but the HTML snapshot captures only empty skeletons, so `extract_from_html` finds 0 usernames. Fix: add a content-depth check — verify post cells contain actual `a[rel="author"]` links before declaring convergence; if zero author links, treat as not-ready and keep polling within the deadline.
|- **Coordinator silent shutdown: workers get CancelledError without wall_halt.set()** — `queue_integration.py:546-727`. In the 14:34:52–14:35:16 run, DRAIN_TRACE loop #10 showed `pending=14786, in_progress=2, busy=[True, True]` (gate NOT met), yet 333ms later both workers received CancelledError and the pipeline exited. No STALL WATCHDOG trigger, no `wall_halt.set()`, no traceback. The `while not wall_halt.is_set()` loop exits via an unlogged path, reaching `asyncio.gather` + `task.cancel()` at lines 725-727. Root cause TBD — needs a 30-second diagnostic run with extra coordinator loop logging.
...
|- CR-1: ~~`probe_page_zero` regex extracts all quoted strings as usernames | `worker.py:649` — reindex skip optimization broken; every blog fully crawled.~~ — RESOLVED 2026-09-10: replaced with Tumblr-specific `@handle` and `data-username` patterns.
...
- CR-4: ~~`navigate_to` render poll has no max-iterations guard | `worker.py:440-460` — infinite loop on non-converging pages.~~ — RESOLVED 2026-09-10: render poll loop is now bounded by `deadline = time.monotonic() + 3.0` (reduced from 12s), which serves as a hard time-based max-iterations guard. Loop exits when deadline passes regardless of poll count.
...
- M-8: ~~`DEAD_PHRASES` matching is case-sensitive~~ — FALSE POSITIVE: all dead-phrase checks in worker.py lowercase the page text/URL before matching (`page_text = html.lower()`, `low_text = cur_text.lower()`, `low_early = early_text.lower()`, `low_url = cur_url.lower()`), and `DEAD_PHRASES` are all lowercase. Matching is already case-insensitive. | Resolved 2026-09-10
...
### 2026-09-10 — CR-1: probe_page_zero regex matched all strings, not just usernames (resolved)
|- **Claim:** `probe_page_zero` used `re.findall(r'\"([^\"]+)\"', html)` which matches every quoted string in the page HTML (post slugs, URLs, timestamps, JSON keys), not just usernames. This made the reindex skip check think usernames were always different from the cached set, so every blog was fully crawled even in reindex mode.
|- **Evidence:** The regex `r'"([^"]+)"'` is a generic JSON string matcher — it has no awareness of Tumblr's username format. On a page with hundreds of JSON strings (post data, API responses embedded in script tags), it extracts dozens of false-positive "usernames" that only happen to start with `@`.
|- **Fix:** Replaced the broad regex with Tumblr-specific patterns: `@([a-zA-Z0-9\-_]+)` matches `@handle` mentions, and `data-username=[\"']([a-zA-Z0-9\-_]+)[\"']` matches `data-username` attribute values. Both are lowercased for comparison against the cached username set. Also added `timeout=10.0` to all 5 `cdp_send` calls in `agent.py:fetch_page_html()` (dead code path, fixed for consistency).
|- **Verification:** `py_compile` clean on `worker.py`, `agent.py`; 48/48 tests pass; no new ruff errors on modified lines.

### 2026-09-10 — Render poll + Page.navigate timeout reductions (worker.py)
|- **Claim:** Session summary (d2, node 293) directed reducing Page.navigate timeout from 15s→5s and render poll convergence cap from 12s→3s, but the current code still had `timeout=15.0` on Page.navigate (line 329) and `deadline = time.monotonic() + 12.0` (line 407). These had not yet been applied.
|- **Evidence:** `worker.py:329` still had `timeout=15.0`; `worker.py:407` still had deadline cap `12.0`; comments at lines 325, 339, 347, 405 still referenced 12s/15s values.
|- **Fix:** Changed `Page.navigate` timeout `15.0` → `5.0` (line 329); changed render poll deadline cap `12.0` → `3.0` (line 407); updated all stale comment/log references from 12s→3s and 15s→5s. Note: the render poll convergence cap of 3s is now the outer loop deadline; individual `cdp_send` calls within the poll already use explicit `timeout=3.0` (line 440) and HTML fetch uses `timeout=10.0` (line 578, unchanged — needs full page HTML which can be large).
|- **Verification:** `py_compile` clean; all 48 tests pass; `ruff check` clean.
